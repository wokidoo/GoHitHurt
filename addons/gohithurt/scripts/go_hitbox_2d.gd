@tool
@icon("res://addons/gohithurt/icons/go_hitbox_2d.svg")
class_name GoHitbox2D extends ShapeCast2D
## A hitbox that scans for [GoHurtbox2D] areas and reports hits.
##
## Each physics frame (subject to [member frame_delay]) this node checks its
## [ShapeCast2D] collisions and, for every overlapping [GoHurtbox2D], emits
## [signal hit_detected] and calls [method GoHurtbox2D.receive_hit]. Colliders
## that are not hurtboxes are added as exceptions so they are skipped on
## subsequent scans.

## Additional data passed to [GoHurtbox2D] on collision.
## [br][b]Note:[/b][br]
...
## Emitted when a hit against a [GoHurtbox2D] is detected. [param collision] is
## the [ShapeCast2D] collision dictionary augmented with a [code]source[/code]
...
func _process_collisions() -> void:
	for collision: Dictionary in collision_result:
		if collision.collider is GoHurtbox2D:
...
## Clears all collision exceptions, then re-adds sibling [GoHurtbox2D] nodes as
## exceptions when [member exclude_parent] is enabled. Extends
## [method ShapeCast2D.clear_exceptions].
func clear_exceptions()->void:
	super()
	if exclude_parent:
		_add_parent_hurtbox_exceptions()
