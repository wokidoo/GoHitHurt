@tool
@icon("res://addons/gohithurt/icons/go_hurtbox_2d.svg")
class_name GoHurtbox2D extends Area2D
## A detectable region that receives hits from a [GoHitbox2D].
##
## A hurtbox is passive: it performs no scanning of its own. It is
## [member Area2D.monitorable] so a [GoHitbox2D]'s shape cast can find it, and
...
## [br][b]Note:[/b] a disabled hurtbox is still [member Area2D.monitorable], so a
## [GoHitbox2D] can still detect it; the hit is simply ignored on arrival rather
...
## forwarded by [GoHitbox2D], including its [code]source[/code] key and any
## merged [member GoHitbox2D.hit_data].
...
## Called by a [GoHitbox2D] when it overlaps this hurtbox. Emits
